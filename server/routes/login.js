...
loginRouter.post("/", (request, response, next) => {
    crypto_1.pbkdf2(request.body.password, user.salt, 10000, config_1.length, config_1.digest, (err, hash) => {
        ...
        if (hash.toString("hex") === user.hashedPassword) {
            const token = jsonwebtoken_1.sign(Object.assign({}, { user: user.username, permissions: [] }), config_1.secret, { expiresIn: "7d" });
...
//# sourceMappingURL=E:/pros/ibm-secure/code-prac/angular2-express-starter-master/dist/server/routes/login.js.map